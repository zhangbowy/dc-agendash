...
const vm = require('vm');
module.exports = function app(agenda, options) {
  options = options || {};
  ...
  const api = async function (
    ...
    apiResponse.title = options.title || "Agendash-DC";
  ...
  const createJob = (jobName, jobSchedule, jobRepeatEvery, jobData) => {
    ...
    agenda.define(jobName, async function (job, done) {
      try {
        ...
        params.sign = getSign(params, options.apisecret),// 约定的签名值
        global.params = JSON.stringify(params);
        const result = vm.runInThisContext(job.attrs.data);
        await result;
        done()
      } catch (err) {
        options.sendFeishu && options.sendFeishu(err);
        done(new Error(err))
      }
    })
   ...
   * 启用、禁用任务
   * @param {*} jobIds 
   * @param {*} enable 目前的禁用的状态
   * @returns 
   */
  const enableJob = (jobIds, enable) => {
    if (!agenda) {
      return Promise.reject(new Error("Agenda instance is not ready"));
    }
    const id = jobIds.map((jobId) => ObjectId(jobId))
    console.log(enable);
    // 如果没有禁用
    if (!enable) {
 ...
 *
 */
 function getSign(data, apiSecret)
 {
