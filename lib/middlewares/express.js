...
module.exports = (agendash) => {
  const { api, requeueJobs, deleteJobs, createJob, enableJob } = agendash;
  const app = express();
  ...
  app.post("/api/jobs/enable", async (request, response) => {
    try {
      const result = await enableJob(request.body.jobIds, request.body.disabled);
      if (result) {
